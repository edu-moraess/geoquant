...
def run_mc(wti0,brt0,bvw,bvb,fcast,ocol,bcol,rbase,rw,rb,vws,vbs,jpu,tdf,bs=1.0,
    ...
    for t in range(steps):
        ...
        eps[:,0]=np.where(vw_>0,sw/vw_,0); eps[:,1]=np.where(vb_>0,sb/vb_,0); eps=np.clip(eps,-5,5)
...
def run_shap(X, y):
    ...
    fig,ax=plt.subplots(figsize=(6,4),facecolor="#FFFFFF"); ax.set_facecolor("#FFFFFF")
...
if needs_run:
    ...
    if vw.empty: vw = pd.Series(pwd, index=returns.index)
    if vb_s.empty: vb_s = pd.Series(pbd, index=returns.index)
    if vg.empty: vg = pd.Series(pgd, index=returns.index)
    ...
    st.session_state.update({
        ...
        "corr_mx":corr_mx,"stress_idx":stress_idx,"feat_imp":feat_imp,"evt":evt_wti,
        "gdiag":gdiag,"bt_res":bt_res,"es_z":es_z,"corr_ewma":corr_ewma,
        "ml_metrics":ml_metrics,"shap_fig":shap_fig,"wf_df":wf_df,"regime":regime,
    })
...
with tab7:
    ...
    with c1:
        bm=S["ml_metrics"]; rows=[]
        for name,vals in bm.items():
            if isinstance(vals.get("RMSE"),(int,float)): rows.append((name,f"{vals['RMSE']:.6f}",f"{vals['MAE']:.6f}"))
            else: rows.append((name,str(vals.get("RMSE","—")),str(vals.get("MAE","—"))))
    ...
    with c2:
        ...
        else: st.info("SHAP matrix interpretation unavailable.")
...
with tab8:
    ...
    else: st.info("Insufficient degrees of freedom for walk-forward validation matrix.")
