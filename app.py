...
def fetch_data(start):
    prices = yf.download(list(TICKERS.values()), start=start, progress=False)
    if isinstance(prices.columns, pd.MultiIndex):
        prices = prices["Close"]
    prices.columns = list(TICKERS.keys())
    for col in prices.columns: prices[col] = fill_gaps(prices[col])
    return prices.ffill().dropna()

@st.cache_data(ttl=60, show_spinner=False)
def fetch_live():
    try:
        wti   = float(yf.Ticker("CL=F").fast_info.get("last_price") or
                      yf.download("CL=F", period="1d", progress=False)["Close"].iloc[-1])
        brent = float(yf.Ticker("BZ=F").fast_info.get("last_price") or
                      yf.download("BZ=F", period="1d", progress=False)["Close"].iloc[-1])
        return wti, brent
    except Exception as e:
        st.warning(f"Live price fetch failed: {e} — using last close.")
        return None, None
