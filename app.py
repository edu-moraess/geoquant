...
def get_usda():
    ...
    try:
        ...
        return {"urea_price":857,"urea_period":"2026-05-12","dap_price":920,
                "dap_period":"2026-05-12","source":"fallback"}
...
def calibrate_weights(returns, prices, gs, fi, sd, window=60):
    ...
    X = pd.DataFrame({
        ...
        "gold_real":gs["gold_real_ret_roll"],"dxy":returns["dxy"].rolling(20).mean(),
        "spread":spread.rolling(20).mean(),"wheat":returns["wheat"].rolling(20).mean(),
        "copper":returns["copper"].rolling(20).mean(),"natgas_vol":returns["natgas"].rolling(20).std(),
        "fert":fi,
    })
    ...
    if len(X2) < window: return GEO_WEIGHTS_DEFAULT.copy()
    ...
    except: return GEO_WEIGHTS_DEFAULT.copy()
...
def run_mc(wti0, brt0, bvw, bvb, fcast, ocol, bcol,
    ...
    for t in range(steps):
        ...
        Rt=np.clip(Rt,-0.9999,0.9999); Rt[:,0,0]=Rt[:,1,1]=1.0
        rho=Rt[:,0,1]; sc=np.sqrt(np.clip(1-rho**2,1e-8,None))
        z=np.random.standard_t(tdf,(sims,2))
        zw=z[:,0]; zb=rho*z[:,0]+sc*z[:,1]
        vw_=np.clip(bvw*ra[:,t],0,max_dv); vb_=np.clip(bvb*ra[:,t],0,max_dv)
    ...
    term=pw[:,-1]; v95=np.percentile(pw[:,1]-wti0,5)
    mask=(pw[:,1]-wti0)<=v95
...
def fetch_data(start):
    """Download market data with multiple fallback strategies."""
    tickers_list = list(TICKERS.values())
    tickers_keys = list(TICKERS.keys())

    # Strategy 1: batch download
    try:
        raw = yf.download(tickers_list, start=start, progress=False,
        ...
        if not raw.empty:
            if isinstance(raw.columns, pd.MultiIndex):
                ...
                if "Close" in raw.columns.get_level_values(0):
                    ...
                    prices = raw.iloc[:, :len(tickers_keys)].copy()
            else:
                prices = raw.copy()

            # Align columns to our key names
            if len(prices.columns) == len(tickers_keys):
                prices.columns = tickers_keys
            else:
                # Column mismatch — try to map by ticker symbol
                col_map = {v: k for k, v in TICKERS.items()}
                prices = prices.rename(columns=col_map)
                prices = prices[[k for k in tickers_keys if k in prices.columns]]

            if not prices.empty and len(prices) > 5:
                for col in prices.columns:
                    prices[col] = fill_gaps(prices[col])
                return prices.ffill().dropna()
    except Exception as e:
        logger.warning(f"Batch download failed: {e}")

    # Strategy 2: individual downloads
    frames = {}
    for key, ticker in TICKERS.items():
        try:
            ...
            logger.warning(f"Individual download failed for {ticker}: {e}")
    ...
    return pd.DataFrame()  # empty — will be caught downstream
...
if needs_run:
    ...
    if prices.empty or len(prices) < 5:
        ...
        st.error("⚠  Não foi possível carregar dados de mercado via yfinance. Aguarde 1 minuto e clique em **Run Full Analysis** novamente.")
        st.stop()
